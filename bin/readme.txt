...
It runs on Windows machines, full screen, and can be used 
in single LCD displays or video walls (see screenshot)

Visually, it emulates old style mechanical plate displays
(where plates cascade sequencialy until the right 
information plate is reached).
In fact, it actually substituted a mechanical display.

Flight information is obtained from http server and 
is parsed from html or xml. 
(Currently it supports Infraero and Solari Udine formats)
...
2- Data files (place in the program bin folder)
...
For each airline in airlines.txt you must provide 
3 bitmaps in the following formats:

(example given for TAM brazilian airline)
TAM.bmp - 20x60  256 color BMP
TAM_GR.bmp - 120x360 256 color BMP
TAM_VW.bmp - 40x100 256 color BMP

(substitute TAM for the airline abreviation)
